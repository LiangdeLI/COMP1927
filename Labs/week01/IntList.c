...
void IntListInsertInOrder(IntList L, int v)
{
	...
	struct IntListNode *t;
        t = L->first;// t is the pointer to the first node
        if (L->size != 0){//if the list is not empty
	    while( t->next != NULL){
	        assert((t->data) <= (t->next->data));
	        t = t->next;
	    }
	}
	...
	assert(L != NULL);
	n = newIntListNode(v);//n is a pointer to the new node with wanted number v
	if (L->size == 0){//add n to the list when the list is empty
	    L->first = L->last = n;
            L->size++;
	}else if (L->size == 1){//add n to the list when the list just have one node
	    
	    if (v <= L->first->data){//v is less than the original number
	        n->next = L->first;
	        L->first = n;
                L->size++;
	    } else {//v is larger than the original number
	        L->last->next = n;
	        L->last = n;
                L->size++;
	    }
	}else {//add n to the list when the list has more than one nodes
	    struct IntListNode *curr = L->first;
	    if (v <= L->first->data){//v is the smallest
	        n->next = L->first;
	        L->first = n;
                L->size++;
	    } else if (v >= L->last->data){//v is the largest
	        L->last->next = n;
	        L->last = n;
                L->size++;
	    } else {//find where n should be
