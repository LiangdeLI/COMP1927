...
static void addType(TransportID type[], TransportID TYPE);
...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   for (curr = g->connections[start]; curr != NULL; curr = curr->next){
       if (curr->v == end){
           ...
           //printf("get boat\n");
           for(check = g->connections[curr->v]; check != NULL; check = check->next){
               if (check->v == end){
                   addType( type, BOAT);
                   //printf("add BOAT\n");
               }
   ...
   int n = 0 ,i;
   for(i = 0; i < 3; i++){
       if (type[i] == ROAD || type[i] == RAIL || type[i] == BOAT)
       n++;
   }
   //printf("%d %d %d\n", type[0], type[1], type[2]);
   return n;  
...
{
   int i, added=0;
   for (i = 0; (i < 3) && (type[i] != TYPE) && !added; i++){
       if (type[i] != ROAD && type[i] != RAIL && type[i] != BOAT){
           type[i] = TYPE;
           added = 1;
       }
